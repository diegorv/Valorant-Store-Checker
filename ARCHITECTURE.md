...
- **Open question — sliding sessions**: sessions do not slide. The cookie's `maxAge` and the JWT's own `exp` are both fixed at sign-in; the row's `expires_at` is pushed back when `getSessionWithRefresh()` re-saves it after a Riot token refresh, but that does not help once the JWT expires. So an active cookie sign-in user is still signed out 30 days after signing in, however much they use the app. A `refreshSession()` that extended the row and the cookie together was written but never called, and was removed as dead code. It also had an expiry defect: it re-set the cookie with the old JWT and left that token's `exp` alone, so the extended cookie would outlive the token and the session would still end on the original date. Removing it did not decide whether sessions should slide; if they should, the refresh has to re-sign the JWT too. `refreshSessionExpiration()` in `session-store.ts` (row only) is still there, with no production caller.
